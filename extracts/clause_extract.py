                ...
                for each_annotation in intervals_swg:
                    ...
                    for token in tokens:
                        if token in self.tags.keys():
                            ...
                            begin_tag = token
                        if not skip:
                            if any(p in token for p in punct): # what is this for???
                                ...
                                word_punct_split = re.findall(r"[^\w\d\s,.!?]*\w+[^\w\d\s,.!?]*\w*[^\w\d\s,.!?]*\w*[^\w\d\s,.!?]*|[^\w\s]", token, re.UNICODE)
                                ...
                                if not clause_annotation:
                                    time_stamp = beg_hms
                                clause_annotation.append(token)
                        if token in self.tags.values() and token == self.tags[begin_tag]: #  is it working?
                            skip = False
                            begin_tag = ''
                for cl in clauses:# why do I need to put this into clauses, for skipping?
                    # add the time stamp and word count first
                    # then
                    #self.output_as_csv(each_file_name[each_file_name.rfind("_") + 1:-9], each_file_name,'SWG', " ".join(cl))
                    beg_hms = cl[-1]
                    # print("time", beg_hms)
                    cl = cl[:-1]
                    # print("cl", cl)
                        ...
                        if word:  # empty word check
                            ...
                            if i + 1 < len(cl):  # make sure next word exist
                                ...
                                if "[REL]" in w_next:
                                    ...
                                    if "wo" in word:
                                        rel_var = " RELd"
                                    elif "als" in word or word.startswith("d") or word.startswith("wel") or word.startswith("jed"):
                                        rel_var = " RELs"
                    ...
                    # columns
                    self.output_as_csv(
                        each_file_name[each_file_name.rfind("_") + 1:-9], beg_hms, sym_seq, " ".join(cl),
                        " ".join(ddm_tags),
                        " ".join(pos_sent))
                    # format the output
                    # skip the story
