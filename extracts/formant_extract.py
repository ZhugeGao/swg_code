...
for speaker_type in speakers:
    ...
    phone_extract_path = working_directory + phone_name
    formant_raw_extract_path = working_directory + formant_name_extract
    formant_path = working_directory + 'done_'+ speaker_type + '/'
    formant_output_path = working_directory + formant_output
    ...
    # # print(df.columns)
    df.to_csv(formant_raw_extract_path, index=False)
    with open(formant_output_path, 'w', newline="") as f:
        ...
        csv_writer.writerow(
         ...
         'word_end_time', 'word_duration', 'word_SWG', 'seg_number', 'zeroed_time_seg', 'normalized_time_seg', 'seg_start_time',
    ...
    formant_extract = read_extract(formant_raw_extract_path)
    ...
    for formant_file in formant_file_list:
        ...
        formant_swg = formant_extract[formant_extract['trans_id'] == formant_file[:-8]] # or split . and take the first part
